...
class DuplicateChecker {
  ...
  async initialize(): Promise<void> {
    ...
    this.taskStore = new PebblesTaskStore(this.projectRoot, db);
    ...
    if (!token) {
      throw new Error('Linear not authenticated. Run "stackmemory linear auth" first.');
    }
  ...
  async runFullScan(): Promise<void> {
    ...
    for (const task of tasks) {
      ...
      spinner.start(`[${progress}%] Checking: ${task.title.substring(0, 50)}...`);

      try {
        // Skip if task already has Linear ID mapped
        if (task.metadata?.linear?.id) {
          spinner.info(`[${progress}%] Skipped (already mapped): ${task.title.substring(0, 40)}...`);
          continue;
        }
        ...
        if (duplicateCheck.isDuplicate && duplicateCheck.existingIssue) {
          ...
          this.report.push({
            ...
            recommendation: duplicateCheck.similarity! > 0.95 ? 'merge' : 
                           duplicateCheck.similarity! > 0.85 ? 'review' : 'skip',
          });
          ...
          spinner.succeed(`[${progress}%] No duplicates: ${task.title.substring(0, 40)}...`);
        }

        // Rate limiting delay
        await this.delay(100);
      } catch (error: unknown) {
        spinner.fail(`[${progress}%] Error checking task: ${(error as Error).message}`);
      }
    }
    ...
    console.log(`  • Check rate: ${Math.round(checked / duration)} tasks/second`);
  }
  ...
  displayReport(): void {
    ...
    const reviewItems = this.report.filter(r => r.recommendation === 'review');
    const skipItems = this.report.filter(r => r.recommendation === 'skip');

    // Display high confidence duplicates (merge)
    if (mergeItems.length > 0) {
      console.log(chalk.red('🔴 High Confidence Duplicates (>95% match) - Recommend Merge:\n'));
  ...
  async saveReport(): Promise<void> {
    ...
    const reportPath = join(this.projectRoot, '.stackmemory', 'duplicate-report.json');
    const markdownPath = join(this.projectRoot, '.stackmemory', 'duplicate-report.md');
  ...
  async suggestActions(): Promise<void> {
    const mergeCount = this.report.filter(r => r.recommendation === 'merge').length;
    const reviewCount = this.report.filter(r => r.recommendation === 'review').length;
    ...
    if (mergeCount > 0 || reviewCount > 0) {
      ...
      if (mergeCount > 0) {
        ...
        console.log(chalk.gray('   stackmemory linear sync --merge-strategy merge_content\n'));
      }

      if (reviewCount > 0) {
        console.log(chalk.yellow('2. Review medium confidence duplicates manually:'));
        console.log(chalk.gray('   Review the duplicate-report.md file and decide per case\n'));
      }

      console.log(chalk.blue('3. Enable duplicate prevention for future syncs:'));
      console.log(chalk.gray('   stackmemory linear sync --daemon --merge-strategy merge_content\n'));
    }
  }
