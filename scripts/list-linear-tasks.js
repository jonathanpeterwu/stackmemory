...
import { LinearAuthManager } from '../dist/src/integrations/linear/auth.js';
...
async function main() {
  try {
    ...
    statusOrder.forEach(statusType => {
      ...
      const matchingKeys = Object.keys(tasksByStatus).filter(key => key.startsWith(statusType));
      ...
      matchingKeys.forEach(statusKey => {
        ...
        tasks.forEach(task => {
          ...
          const titleStr = task.title.length > 50 
            ? task.title.substring(0, 47) + '...' 
            : task.title;
          
          console.log(`  ${chalk.cyan(task.identifier.padEnd(8))} ${titleStr.padEnd(50)} ${priorityStr.padEnd(8)} ${assigneeStr}`);
        });
      });
    });
