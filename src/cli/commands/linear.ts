      ...
      try {
        ...
        if (options.status) {
          ...
          if (!targetState) {
            ...
            states.forEach(s => console.log(chalk.gray(`  - ${s.name} (${s.type})`)));
            return;
          }
        ...
        console.log(chalk.green(`✓ Updated ${updatedIssue.identifier}: ${updatedIssue.title}`));
        if (options.status) {
          console.log(chalk.cyan(`  Status: ${updatedIssue.state.name}`));
        }
        console.log(chalk.gray(`  ${updatedIssue.url}`));
      } catch (error) {
        console.error(chalk.red('Failed to update task:'), (error as Error).message);
      }
    });
