...
export class BrowserMCPIntegration {
      ...
      try {
        switch (name) {
          case 'browser_navigate':
            return await this.navigate(String(args.url), args.sessionId as string);
          
          case 'browser_screenshot':
            return await this.screenshot(String(args.sessionId), args.fullPage as boolean, args.selector as string);
          
          case 'browser_click':
            return await this.click(String(args.sessionId), String(args.selector));
          
          case 'browser_type':
            return await this.type(String(args.sessionId), String(args.selector), String(args.text));
          
          case 'browser_evaluate':
            return await this.evaluate(String(args.sessionId), String(args.script));
          
          case 'browser_wait':
            return await this.waitFor(String(args.sessionId), args.selector as string, args.timeout as number);
          
          case 'browser_get_content':
            return await this.getContent(String(args.sessionId), args.selector as string);
   ...
   */
  private async type(sessionId: string, selector: string, text: string): Promise<any> {
    const session = this.getSession(sessionId);
    if (!session) {
      throw new Error(`Session ${sessionId} not found`);
    }
  ...
  private async getOrCreateSession(sessionId?: string): Promise<BrowserSession> {
    ...
    const browser = await puppeteer.launch({
      ...
      defaultViewport: this.config.defaultViewport || { width: 1280, height: 720 },
